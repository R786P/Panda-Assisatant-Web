...
class LiveScreenService : Service() {
    ...
    private val recentFrames=java.util.concurrent.CopyOnWriteArrayList<String>()
    ...
    override fun onStartCommand(i:Intent?,f:Int,s:Int):Int{
        ...
        startProjection(code,data,true);return START_STICKY
    }
    ...
    private fun startProjection(code:Int,data:Intent,showBubble:Boolean){
    ...
    private fun capture(im:android.media.Image?){
        ...
        try{
            ...
            val out=ByteArrayOutputStream();sc.compress(Bitmap.CompressFormat.JPEG,50,out)
            val encoded=Base64.encodeToString(out.toByteArray(),Base64.NO_WRAP)
            latestFrame=encoded
            recentFrames.add(encoded)
            while(recentFrames.size>3)recentFrames.removeAt(0)
    ...
    private fun closeFloating(){closeWeb();try{bubble?.let{if(it.parent!=null)wm?.removeView(it)}}catch(_:Exception){};bubble=null;stopScreenCapture();stopSelf()}
    private fun closeWeb(){try{webRoot?.let{if(it.parent!=null)wm?.removeView(it)};web?.stopLoading();web?.destroy()}catch(_:Exception){};web=null;webRoot=null;webLp=null;stopScreenCapture()}
    ...
    private fun openWeb(startVoice:Boolean){
        ...
        if(projection==null && projectionCode!=0 && projectionData!=null){
            if(Build.VERSION.SDK_INT>=29)startForeground(ID,notification(),android.content.pm.ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION) else startForeground(ID,notification())
            startProjection(projectionCode,projectionData!!,false)
        }
        ...
        web=WebView(this).apply{
            ...
            addJavascriptInterface(object{
                ...
                @JavascriptInterface fun screenFrames():String=org.json.JSONArray(recentFrames).toString()
            ...
            webViewClient=object:WebViewClient(){override fun onPageFinished(v:WebView,u:String){super.onPageFinished(v,u);v.evaluateJavascript("""
                (function(){const oldFetch=window.fetch;window.fetch=function(url,opt){try{if(String(url).includes('/api/chat')&&opt&&opt.body){const d=JSON.parse(opt.body);const s=window.PandaNative?window.PandaNative.screen():'';const fs=window.PandaNative?window.PandaNative.screenFrames():'[]';if(s){d.image=s;d.mime_type='image/jpeg'}try{const arr=JSON.parse(fs);if(arr.length)d.frames=arr.slice(-3)}catch(e){}opt.body=JSON.stringify(d)}}catch(e){}return oldFetch(url,opt)}})();
