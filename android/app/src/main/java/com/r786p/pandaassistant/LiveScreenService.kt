...
class LiveScreenService : Service() {
    ...
    override fun onCreate(){super.onCreate();getSystemService(NotificationManager::class.java).createNotificationChannel(NotificationChannel(CH,"Panda Live Screen",NotificationManager.IMPORTANCE_LOW))}
                    ...
                    v.evaluateJavascript("""
                        (function(){
                          ...
                          window.fetch=function(url,opt){
                            try{
                              if(String(url).includes('/api/chat') && opt && opt.body){
                                const d=JSON.parse(opt.body); const s=window.PandaNative?window.PandaNative.screen():'';
                                if(s){d.image=s;d.mime_type='image/jpeg';opt.body=JSON.stringify(d);}
                              }
                            }catch(e){}
    ...
    private class PandaBubble(c:Context):View(c){
        private val p=Paint(1);var tap:(()->Unit)?=null;var mic:(()->Unit)?=null;var move:((Float,Float)->Unit)?=null
        var sx=0f;var sy=0f;var drag=false
        override fun onTouchEvent(e:MotionEvent):Boolean{when(e.actionMasked){
            MotionEvent.ACTION_DOWN->{sx=e.x;sy=e.y;drag=false;return true}
            MotionEvent.ACTION_MOVE->{val dx=e.x-sx;val dy=e.y-sy;if(dx*dx+dy*dy>100){drag=true;move?.invoke(dx,dy);sx=e.x;sy=e.y};return true}
            MotionEvent.ACTION_UP->{if(!drag){val cx=width*.72f;val cy=height*.78f;val r=width*.20f;if((e.x-cx)*(e.x-cx)+(e.y-cy)*(e.y-cy)<r*r)mic?.invoke()else tap?.invoke()};return true}
        };return true}
